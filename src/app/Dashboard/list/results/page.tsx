...
const ResultRow = ({ item }: { item: ResultInfo }) => {
...
const ResultList = () => {
  ...
  useEffect(() => {
    ...
    const loadMeta = async () => {
      try {
        ...
        const sList = Array.isArray(sJson?.subjects)
          ? sJson.subjects
          : Array.isArray(sJson)
          ? sJson
          : [];
        setSubjects(sList.map((s: any) => ({ id: s.id, name: s.name })));
        const tList = Array.isArray(tJson?.teachers)
          ? tJson.teachers
          : Array.isArray(tJson)
          ? tJson
          : [];
    ...
    const fetchResults = async () => {
      try {
        const qs = [
          ...
          .filter(Boolean)
          .join("&");
        const response = await fetch(`/api/results?${qs}`);
        const data = await response.json();
        setResults(data.results || data);
        setTotalPages(
          data.totalPages ||
            Math.ceil((data.results || data).length / itemsPerPage)
        );
        setTotalItems(data.totalItems || (data.results || data).length);
      } catch (error) {
  ...
  const rowData = (item: ResultInfo) => <ResultRow key={item.id} item={item} />;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 text-gray-500 w-full md:w-auto">
                ...
                <div className="absolute right-0 mt-2 w-72 bg-white border border-gray-200 rounded-md shadow-lg z-10 p-3 space-y-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setAssessmentType((e.target.value as any) || "")
                      }
                      className="w-full p-2 text-sm ring-[1.5px] ring-gray-300 rounded-md bg-white"
                    >
            ...
            <div className="flex items-center gap-4 self-end">
              ...
              {isAdmin && <ActionForm table="Result" type="create" />}
            </div>
          </div>
        </div>
        <Table columns={columns} rowData={rowData} data={results} />
        <Pagination
