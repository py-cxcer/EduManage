...
const AssignmentRow = ({ item }: { item: AssignmentInfo }) => {
...
const AssignmentList = () => {
    ...
    const fetchAssignments = async () => {
      try {
        const qs = [
          ...
          .filter(Boolean)
          .join("&");
        const response = await fetch(`/api/assignments?${qs}`);
        const data = await response.json();
        setAssignments(data.assignments || data);
        setTotalPages(
          data.totalPages ||
            Math.ceil((data.assignments || data).length / itemsPerPage)
        );
        setTotalItems(data.totalItems || (data.assignments || data).length);
      } catch (error) {
  ...
  const rowData = (item: AssignmentInfo) => (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 text-gray-500 w-full md:w-auto">
            ...
            <div className="flex items-center gap-4 self-end">
              ...
              {isAdmin && <ActionForm table="Assignment" type="create" />}
            </div>
          </div>
        </div>
        <Table columns={columns} rowData={rowData} data={assignments} />
        <Pagination
