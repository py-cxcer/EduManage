  ...
  return (
    ...
    >
      <td className="flex items-center gap-4 p-4">
        <div>
          <h3 className="font-semibold text-gray-700">{item.subjectName}</h3>
        </div>
      </td>
...
const LessonList = () => {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      try {
        ...
        setTotalItems(data.totalItems || (data.lessons || data).length);
      } catch (error) {
  ...
  const rowData = (item: LessonInfo) => <LessonRow key={item.id} item={item} />;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 text-gray-500 w-full md:w-auto">
            ...
            <div className="flex items-center gap-4 self-end">
              {isAdmin && <ActionForm table="Lesson" type="create" />}
            </div>
          </div>
        </div>
        <Table columns={columns} rowData={rowData} data={lessons} />
        <Pagination
